...
export class PokemonService {
  ...
  getPokemons():Observable<Pokemon[]> {
    return this.http.get<PokemonResponse>(`${environment.api_base_url}/pokemon/`, {params: this.params}).pipe(
  ...
  getPokemonDetails(name:string): Observable<PokemonDetailsReponse> {
    return this.http.get<PokemonDetailsReponse>(`${environment.api_base_url}/pokemon/${name}`)
  }
  ...
  getPaginationPrevious(previous: number) {
    ...
    return this.http.get<PokemonResponse>(`${environment.api_base_url}/pokemon/?limit=20&offset=${this.offsetPage}`).pipe(
      map(resp => resp.results),
      catchError(error => of([]))
    )
  }
